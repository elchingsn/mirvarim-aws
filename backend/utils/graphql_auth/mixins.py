...
from django.contrib.auth.models import Group #new addition
        ...
        try:
            with transaction.atomic():
                ...
                if f.is_valid():
                    ...
                    # new addition: add users to group during registration
                    role = kwargs.get(UserModel.ROLE_FIELD, "1")
                    if role == "1":
                        visiter = Group.objects.get(name="visiter")  
                        user.groups.add(visiter)
                    if role == "2":
                        freelancer = Group.objects.get(name="freelancer")  
                        user.groups.add(freelancer)
                    if role == "3":
                        salonAdmin = Group.objects.get(name="salonAdmin")  
                        user.groups.add(salonAdmin)
