...
import {SELECTED_SALON_QUERY} from "pages/SalonDetail"

export default function Booking({ match }) {
  const { t } = useTranslation();
  const id = match.params.id;

  const { data } = useQuery(SELECTED_SALON_QUERY, {variables: {id}});
  ...
  return <BookingForm salon={salon} services={services} />

}

function BookingForm({services, salon, currentUser}) {
